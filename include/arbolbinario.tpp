...
unsigned int ArbolBinario<T>::numero_nodos(const ArbolBinario<T>::info_nodo *d) const
{
	if (d == 0)
		...
		return 1 + numero_nodos(d->hizq) + numero_nodos(d->hder);
}
...
bool ArbolBinario<T>::iguales(const ArbolBinario<T>::info_nodo *s1, const ArbolBinario<T>::info_nodo *s2) const
{
	...
	if (s1->et != s2->et)
		...
		return iguales(s1->hizq, s2->hizq) && iguales(s1->hder, s2->hder);
}

template <class T>
void ArbolBinario<T>::InsertarHijoIzquierda(ArbolBinario<T>::info_nodo *n, ArbolBinario<T>::info_nodo *sub)
{
	if (sub != 0)
	{
		BorrarInfo(n->hizq);
...
template <class T>
void ArbolBinario<T>::InsertarHijoIzquierda(ArbolBinario<T>::info_nodo *n, const T &e)
{
...
template <class T>
void ArbolBinario<T>::InsertarHijoDerecha(ArbolBinario<T>::info_nodo *n, ArbolBinario<T>::info_nodo *sub)
{
	if (sub != 0)
	{
		BorrarInfo(n->hder);
...
template <class T>
void ArbolBinario<T>::InsertarHijoDerecha(ArbolBinario<T>::info_nodo *n, const T &e)
{
	info_nodo *nuevo = new info_nodo(e);
...
template <class T>
void ArbolBinario<T>::Sustituye_Subarbol(typename ArbolBinario<T>::nodo pos_this, const ArbolBinario<T> &a, typename ArbolBinario<T>::nodo pos_a)
{
	if (this != &a)
	{
		if (pos_this.p == raiz)
		...
		else
		{
			...
			if (padre->hizq == pos_this.p)
			{ //si es el hijo a la izquierda
